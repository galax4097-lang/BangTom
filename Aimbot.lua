...
-- Cài đặt mục ESP (Bổ sung thanh kéo m phát sáng và ẩn hiện)
...
local function ManageTextESP(player)
    ...
    local function createGui(char)
        ...
        connection = RunService.RenderStepped:Connect(function()
            ...
            -- KIỂM TRA ĐIỀU KIỆN KHOẢNG CÁCH: Nếu vượt quá mét quy định thì ẩn chữ hoàn toàn
            if dist > Config.ESP.MaxDistance or (Config.ESP.TeamCheck and player.Team == LocalPlayer.Team) then
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if head and hum and hum.Health > 0 then
                ...
                -- ĐIỀU KIỆN KHÓA: Nếu nhân vật nằm ngoài tầm hiển thị của ESP thì không tự động AIM
...
RunService.RenderStepped:Connect(function()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
            ...
            -- Nếu TẮT ESP, hoặc LỌC ĐỒNG ĐỘI, hoặc VƯỢT QUÁ SỐ MÉT QUY ĐỊNH -> Xóa phát sáng ngay lập tức
            if not Config.ESP.Enabled or (Config.ESP.TeamCheck and player.Team == LocalPlayer.Team) or dist > Config.ESP.MaxDistance then
                if oldHl then oldHl:Destroy() end
                continue
            end

            -- Nếu thỏa mãn khoảng cách thiết lập, tạo hiệu ứng phát sáng Chams
